...
CREATE TABLE `Birthday` (
    ...
    UNIQUE INDEX `Birthday_userId_key`(`userId`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;

-- CreateTable
CREATE TABLE `BirthdayCelebrate` (
    `id` INTEGER NOT NULL AUTO_INCREMENT,
    `createdAt` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `birthDayId` INTEGER NOT NULL,
    `celebraterId` VARCHAR(191) NOT NULL,
...
-- AddForeignKey
ALTER TABLE `BirthdayCelebrate` ADD CONSTRAINT `BirthdayCelebrate_birthDayId_fkey` FOREIGN KEY (`birthDayId`) REFERENCES `Birthday`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `BirthdayCelebrate` ADD CONSTRAINT `BirthdayCelebrate_celebraterId_fkey` FOREIGN KEY (`celebraterId`) REFERENCES `User`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;
